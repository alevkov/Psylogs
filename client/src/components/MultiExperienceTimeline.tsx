...
// Dutch fields color palette is imported from color-utils
// We'll generate colors dynamically based on substance name

export function MultiExperienceTimeline({ className }: MultiExperienceTimelineProps) {
  ...
  const [activeExperiences, setActiveExperiences] = useState<ActiveExperience[]>([]);
  const [lastUpdate, setLastUpdate] = useState<Date>(new Date());
  
  // Helper function to find timeline data for a substance and route
  const findTimelineData = useCallback((substance: string, route: string): TimelineData | null => {
    ...
    try {
      ...
      const activeDoses = doses
        .filter(dose => {
          ...
          const totalDurationHours = timelineDrug.duration_curve.after_effects.end;
      ...
      const experiences = activeDoses.map((dose, index) => {
        const timelineDrug = findTimelineData(dose.substance, dose.route);
        ...
        const doseTime = new Date(dose.timestamp);
        const elapsedTimeHours = differenceInMilliseconds(now, doseTime) / 3600000; // convert to hours
        
        // Calculate offset from earliest dose in hours
        const offsetFromEarliestHours = differenceInMilliseconds(doseTime, earliestDoseTime) / 3600000;
  ...
  const generateIntensityCurve = useCallback((timeline: TimelineData, totalDurationHours: number, pointCount = 200): Point[] => {
    const points: Point[] = [];
    
    // Safeguard against invalid timeline data
    if (!timeline?.duration_curve) {
      console.warn("Invalid timeline data in MultiExperienceTimeline:", timeline);
      return [];
    }
  ...
  const drawTimelineCurves = useCallback(() => {
    ...
    const latestEndTime = Math.max(
      ...activeExperiences.map(exp => {
        const totalExperienceDuration = exp.timeline.duration_curve.after_effects.end; // Use the end of after-effects as total duration
    ...
    for (let i = 0; i < intensityLabels.length; i++) {
      const y = height - paddingY - (i / (intensityLabels.length - 1)) * graphHeight;
      context.fillText(intensityLabels[i], 2, y + 3);
    }
    
    // Draw the "now" line
    const nowOffsetHours = differenceInMilliseconds(now, earliestDoseTime) / 3600000;
    if (nowOffsetHours <= totalTimelineDuration) {
      const nowX = paddingX + (nowOffsetHours / totalTimelineDuration) * graphWidth;
    ...
    activeExperiences.forEach(experience => {
      ...
      points.forEach((point, index) => {
        ...
        if (point.x <= timeline.duration_curve.after_effects.start) {
          if (index === 0 || (index > 0 && points[index-1].x <= timeline.duration_curve.after_effects.start)) {
            if (index === 0) {
              context.moveTo(x, y);
            } else {
              context.lineTo(x, y);
      ...
      const experienceStartX = paddingX + (offsetFromEarliestHours / totalTimelineDuration) * graphWidth;
      ...
      const endX = paddingX + ((offsetFromEarliestHours + totalExperienceDuration) / totalTimelineDuration) * graphWidth;
      ...
      if (elapsedTimeHours <= totalExperienceDuration) {
        ...
        const currentX = paddingX + (currentAdjustedX / totalTimelineDuration) * graphWidth;
        ...
        for (let i = 1; i < points.length; i++) {
          if (points[i].x >= elapsedTimeHours) {
            ...
            const ratio = (elapsedTimeHours - prevPoint.x) / (nextPoint.x - prevPoint.x);
    ...
    // No need for phase labels here since we're showing multiple substances with different phase timings
    // Each substance has its own onset/peak/offset times that would get confusing
    // Instead, we use the "current position" markers on each curve to show the current phase
            ...
            <div className="min-w-full overflow-hidden rounded-md border text-xs">
              <table className="w-full">
                <thead className="bg-muted/50">
                  <tr>
                    <th className="p-1.5 pl-2 text-left font-medium">Substance</th>
                    <th className="p-1.5 text-center font-medium whitespace-nowrap">Time ago</th>
                ...
                <tbody>
                  {activeExperiences.map((exp, idx) => {
                    ...
                    return (
                      <tr key={exp.dose.id} className={idx % 2 ? "bg-muted/20" : ""}>
                        ...
                        <td className="p-1.5 pr-2 text-right">
                          <Badge 
                            variant={
                              exp.phase === "onset" ? "default" : 
                              exp.phase === "peak" ? "destructive" : 
                              exp.phase === "offset" ? "secondary" : "outline"
                            }
                            className="text-[10px] font-normal whitespace-nowrap"
                          >
                            {exp.phase === "onset" ? "Coming up" : 
                             exp.phase === "peak" ? "Peak" : 
                             exp.phase === "offset" ? "Offset" : "After"}
                          </Badge>
                        </td>
                      </tr>
                    );
